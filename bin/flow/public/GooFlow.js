...
GooFlow.prototype = {
    ...
    addNode: function (id, json) {
        ...
        json.width = json.width ? json.width : 170;
    ...
    setName: function (id, name, type) {
        ...
        if (type == "node") {//如果是结点
            ...
            let label = '';
            if (this.$lineData[id].def) {
                label += '[默]';
            }
            if (this.$lineData[id].error) {
                label += '[异]';
            }
            if (this.$lineData[id].hide) {
                label += '[隐]';
            }
            if (this.$lineData[id].right) {
                label += '[正]';
            }
            ...
            if (GooFlow.prototype.useSVG != "") {
                ...
                this.$lineDom[id].childNodes[1].innerHTML = name + label;
    ...
    addLine: function (id, json) {
        ...
        this.$lineData[id].blackList = json.blackList;
        this.$lineData[id].whiteList = json.whiteList;
        ...
        else this.$lineDom[id].childNodes[2].textContent = json.name + label;
        ++this.$lineCount;
    },
    ...
    resetLines: function (id, node) {
        for (var i in this.$lineData) {
            ...
            let label = '';
            if (this.$lineData[i].def) {
                label += '[默]';
            }
            if (this.$lineData[i].error) {
                label += '[异]';
            }
            if (this.$lineData[i].hide) {
                label += '[隐]';
            }
            if (this.$lineData[i].right) {
                label += '[正]';
            }
            ...
            else this.$lineDom[i].childNodes[2].textContent = this.$lineData[i].name + label;
        }
    },
    ...
    setLineType: function (id, newType) {
        ...
        //如果是变回直线
            ...
            this.$draw.appendChild(this.$lineDom[id]);
            if (GooFlow.prototype.useSVG == "") {
                this.$lineDom[id].childNodes[1].innerHTML = this.$lineData[id].name + label;
                this.$lineDom[id].childNodes[1].style.left =
                    ((res.end[0] - res.start[0]) * (res.end[0] > res.start[0] ? 1 : -1) - this.$lineDom[id].childNodes[1].offsetWidth) / 2 + 4;
            }
            else
                this.$lineDom[id].childNodes[2].textContent = this.$lineData[id].name + label;
        }
        if (this.$focus == id) {
            this.focusItem(id);
        }
    ...
    setLineM: function (id, M, noStack) {
        ...
        else this.$lineDom[id].childNodes[2].textContent = this.$lineData[id].name + label;
    },
    //删除转换线
    delLine: function (id) {
        if (!this.$lineData[id]) return;
